...
def create_callback(algo, save_path, verbose=1):
    ...
    def sac_callback(_locals, _globals):
        ...
        if len(episode_rewards[-101:-1]) == 0:
            ...
            print("episode reward: {}, [-101:-1]: {}".format(episode_rewards, episode_rewards[-101:-1]))
    ...
    def ppo_callback(_locals, _globals):
        """
        ...
        """
        global best_mean_reward
        print("ここでエラー _locals: {}".format(_locals.keys()))
        episode_rewards = _locals['episode_rewards']
        if len(episode_rewards[-101:-1]) == 0:
            return True
        else:
